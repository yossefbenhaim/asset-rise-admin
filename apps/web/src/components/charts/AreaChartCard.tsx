    ...
    <ChartCard title={title} sub={sub} index={index}>
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
          ...
          <CartesianGrid strokeDasharray="4 4" stroke={gridStroke} strokeOpacity={0.6} vertical={false} />
          <XAxis dataKey={xKey} {...axisProps} minTickGap={24} />
          <YAxis {...axisProps} width={40} allowDecimals={false} />
          <Tooltip content={<ChartTooltip valueFmt={valueFmt} />} cursor={{ stroke: color, strokeOpacity: 0.25, strokeWidth: 1.5 }} />
          <Area
            type="monotone"
            dataKey={yKey}
            stroke={`url(#${sid})`}
