...
function Pill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[11px] leading-none px-2 py-1 rounded-md font-medium ${TONE_CLS[tone]}`}
    >
      {icon}
      {children}
    </span>
  ...
  return (
    <div className="rounded-xl border border-sky-200 bg-sky-50 p-3 sm:p-4">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {r.plan_number && (
          <span className="font-mono text-[11px] text-slate-500">{r.plan_number}</span>
        )}
        <span className="ms-auto text-[11px] px-2 py-1 rounded bg-sky-600 text-white font-semibold">
          {STAGE_HE[r.stage ?? ''] ?? r.stage ?? 'רץ'}
        </span>
      </div>
      {r.stage === 'downloading' && (
        <div className="text-sm text-sky-800 mt-2">
          מוריד קבצים: <b className="tabular-nums">{r.downloaded_count ?? 0}</b>
          {r.last_file ? <span className="text-slate-500 break-all"> · {r.last_file}</span> : null}
        </div>
      )}
        ...
        {steps.slice(-5).map((s, i) => (
          <li key={i} className="text-[11px] text-slate-500">
            <span className="tabular-nums">
              {new Date(s.t).toLocaleTimeString('he-IL', {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit',
              })}
...
type Search = {
  ...
  has_plan_page: boolean | null
  economics_source: string | null
  docs_pending: boolean | null
}
const AI_HE: Record<string, string> = {
  done: 'הושלם',
  pending: 'ממתין',
  running: 'רץ',
  disabled: '—',
...
function searchToRun(s: Search): Run {
  return {
    ...
    gush: s.gush,
    zips_downloaded: null,
    pdfs_extracted: null,
    docs: null,
    economics_found: s.has_economics,
    status: 'cache',
    duration_s: null,
    stage: null,
    steps: null,
...
function SearchCard({ s, onClick }: { s: Search; onClick?: () => void }) {
  ...
  const aiTone: Tone =
    s.ai_status === 'done' ? 'emerald' : s.ai_status === 'failed' ? 'rose' : 'sky'
  const meta = [
  ...
  return (
    <div
      onClick={onClick}
      className="rounded-lg border border-slate-200 bg-white p-3 cursor-pointer hover:border-sc-primary/50 hover:shadow-sm transition-all"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="font-semibold text-sm truncate">{addr(s)}</div>
          <div className="text-[11px] text-slate-400 tabular-nums mt-0.5 truncate">
            {shortTime(s.created_at)}
            {meta ? ` · ${meta}` : ''}
          </div>
        </div>
        <span className="shrink-0 text-sm font-bold tabular-nums px-2.5 py-1 rounded-lg bg-emerald-600 text-white">
          {s.score ?? '—'}
        </span>
      </div>
      <div className="flex flex-wrap gap-1.5 mt-2.5">
        {fresh ? (
          <Pill tone="sky" icon={<Zap size={11} />}>
            חיפוש טרי
          </Pill>
        ) : (
          <Pill tone="amber" icon={<Database size={11} />}>
            מטמון {(s.cache_level ?? '').toUpperCase()}
          </Pill>
        )}
        <Pill tone={aiTone} icon={<Sparkles size={11} />}>
          AI {AI_HE[s.ai_status ?? ''] ?? s.ai_status ?? '—'}
        </Pill>
        {s.docs_pending ? (
          <Pill tone="sky" icon={<Loader2 size={11} className="animate-spin" />}>
            מוריד מסמכים
          </Pill>
        ) : s.has_economics ? (
          <Pill tone="violet" icon={<Coins size={11} />}>
            נתונים כלכליים · {s.economics_source === 'ai' ? 'AI' : 'מהמסמכים'}
          </Pill>
        ) : (
...
const DOC_TYPE_HE: Record<string, string> = {
  ...
  binui: 'נספח בינוי',
  balance: 'טבלת איזון והקצאה',
  traffic: 'נספח תנועה',
  social: 'נספח חברתי',
  environment: 'נספח סביבה',
  takanon: 'תקנון',
  tasrit: 'תשריט',
  other: 'אחר',
  unknown: 'לא מסווג',
...
function DBlock({
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-3.5">
      <h3 className="flex items-center gap-1.5 text-[13px] font-semibold text-slate-700 mb-3">
        {icon}
        {title}
      </h3>
      {children}
    ...
    <Drawer
      ...
      title={
        <span className="flex items-center gap-2">
          <MapPin size={16} className="text-sc-primary" />
          {addr(run)}
          <span className={`text-[11px] px-2 py-0.5 rounded font-semibold ${st.cls}`}>
            {st.label}
          </span>
        </span>
      }
      ...
      <div className="space-y-3">
        ...
        <DBlock icon={<Database size={14} className="text-sky-600" />} title="מקור הנתונים">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="מספר תכנית">
              <span className="font-mono text-xs">{run.plan_number || '—'}</span>
            </Field>
            <Field label="mp_id">
              <span className="font-mono text-xs">{run.mp_id || '—'}</span>
            </Field>
            <Field label="גוש / חלקה">
              {run.gush ?? '—'}
              {search?.chelka ? ` / ${search.chelka}` : ''}
            </Field>
            <Field label="מטמון / טרי">
              {search
                ? fresh
                  ? 'חיפוש טרי (חושב עכשיו)'
                  : `מטמון ${(search.cache_level ?? '').toUpperCase()}`
                : '—'}
            </Field>
            <Field label="תיק תכנית (מבא״ת)">{search?.has_plan_page ? 'נסרק ✓' : '—'}</Field>
            <Field label="ציון סופי">
              {search?.score != null ? <b className="text-emerald-700">{search.score}</b> : '—'}
              {search?.bucket ? (
                <span className="text-slate-400 text-xs"> · {search.bucket}</span>
              ) : (
                ''
              )}
            </Field>
        ...
        <DBlock icon={<Timer size={14} className="text-violet-600" />} title="תזמון">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="הסתיים">
              {run.finished_at ? fullTime(run.finished_at) : 'עדיין רץ…'}
            </Field>
            <Field label="משך כולל">
              <b>{fmtDur(run.duration_s)}</b>
            </Field>
        ...
        <DBlock icon={<Download size={14} className="text-sky-600" />} title="הורדות מ-MAVAT">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="נתונים כלכליים">
              {econSrc ? (
                <span className="text-violet-700 font-medium">{econSrc}</span>
              ) : (
                'לא נמצאו'
              )}
            </Field>
        ...
        <DBlock
          ...
          title={`מסמכים (${docs.length}) · ${read.length} נקראו · ${skipped.length} דולגו`}
        >
          {docs.length === 0 ? (
            <div className="text-sm text-slate-400">אין מסמכים.</div>
          ) : (
            <ul className="space-y-1.5">
              {docs.map((d, i) => (
                <li key={i} className="flex items-center gap-2 text-sm">
                  ...
                  {d.pii_removed > 0 && (
                    <span className="ms-auto text-[11px] text-amber-600">
                      {d.pii_removed} פרטים אישיים הוסרו
                    </span>
                  )}
                </li>
              ))}
        ...
        <DBlock
          ...
          title={`ציר זמן מלא (${steps.length} שלבים)`}
        >
          {steps.length === 0 ? (
            <div className="text-sm text-slate-400">אין שלבים מתועדים.</div>
          ) : (
            <ol className="border-s-2 border-slate-200 ps-3 space-y-1.5">
              {steps.map((s, i) => {
                const prev = i > 0 ? steps[i - 1] : null
                const gap = prev
                  ? Math.round((new Date(s.t).getTime() - new Date(prev.t).getTime()) / 1000)
                  : 0
                return (
        ...
        <div className="text-[10px] text-slate-300 font-mono break-all pt-1">
          run {run.id}
          {run.job_id ? ` · job ${run.job_id}` : ''}
        </div>
      </div>
    </Drawer>
  )
...
export default function AdminPipelineRuns() {
  ...
  const list = trpc.pipelineRuns.list.useQuery(
    { limit: 500 },
    { refetchInterval: 4000, refetchOnWindowFocus: true },
  )
  const searchList = trpc.pipelineRuns.searches.useQuery(
    { limit: 200 },
    { refetchInterval: 4000, refetchOnWindowFocus: true },
  )
  ...
  const sameAddr = (
    a: { city: string | null; street: string | null; building_number: string | null },
    b: typeof a,
  ) => addr(a) === addr(b)
  const openRun = (run: Run) =>
    setActive({
      ...
      search: searches.find(
        s =>
          sameAddr(s, run) &&
          (!run.plan_number || !s.plan_number || s.plan_number === run.plan_number),
      ),
    })
  const openSearch = (s: Search) => {
    const run = rows.find(
      r => sameAddr(r, s) && (!s.plan_number || !r.plan_number || r.plan_number === s.plan_number),
    )
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <span className="tabular-nums text-xs text-slate-500">
            {shortTime(row.original.created_at)}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-mono text-xs">{row.original.plan_number || '—'}</span>
        ),
      },
      {
        header: 'שלב',
        accessorKey: 'stage',
        cell: ({ row }) => (
          <span className="text-xs">{STAGE_HE[row.original.stage ?? ''] ?? '—'}</span>
        ),
      },
      {
        header: 'קבצים',
        ...
        cell: ({ row }) => (
          <span className="tabular-nums text-xs">
            {row.original.pdfs_extracted ?? 0} PDF · {row.original.zips_downloaded ?? 0} ZIP
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const types = [...new Set(docs.filter(d => d.ai_visible).map(d => d.type))]
            .slice(0, 6)
            .join(' · ')
          return (
            <div className="text-xs" title={types}>
              <span className="text-emerald-700 font-semibold">{read} נקראו</span>
              {skipped > 0 && <span className="text-slate-400"> · {skipped} דולגו</span>}
      ...
      {
        header: 'נתונים כלכליים',
        id: 'econ',
        accessorFn: r => (r.economics_found ? 1 : 0),
        cell: ({ row }) =>
          row.original.economics_found ? (
      ...
      },
      {
        ...
        accessorKey: 'status',
        cell: ({ row }) => {
          const s = STATUS_HE[row.original.status ?? 'started'] ?? STATUS_HE.started
          return (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="tabular-nums text-xs text-slate-500">
            {row.original.duration_s != null ? `${row.original.duration_s}ש׳` : '—'}
          </span>
        ),
      },
    ],
    ...
    <div className="sc-page">
      <div className="sc-page__head">
        ...
        <p className="text-sm text-slate-500">
          שליפת מסמכי תכנית דרך ה-VPN הישראלי בזמן אמת. מתעדכן אוטומטית · ללא פרטים אישיים.
        </p>
      </div>
      ...
      {live.length > 0 && (
        <div className="mb-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-sky-700 mb-2">
            ...
            רץ עכשיו ({live.length})
          </div>
          <div className="space-y-2.5">
            {live.map(r => (
              <LiveRun key={r.id} r={r} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="mb-4">
        ...
        {searches.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-200 text-sm text-slate-400 py-8 text-center">
            אין חיפושים עדיין.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 max-h-[520px] overflow-y-auto pe-0.5">
            {searches.map(s => (
              <SearchCard key={s.id} s={s} onClick={() => openSearch(s)} />
            ))}
          </div>
        )}
      </div>
      ...
      {active && (
        <RunDetail run={active.run} search={active.search} onClose={() => setActive(null)} />
      )}
    </div>
  )
}
