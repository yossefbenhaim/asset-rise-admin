    ...
    >
      <div className="p-4 space-y-1 overflow-y-auto">
        ...
        <Section title="תפקיד ומטרה" icon={<Bot size={13} />}>
          ...
          <p className="text-[13px] text-sc-text-secondary leading-relaxed m-0">
            {agent.purpose ?? '—'}
          </p>
          {agent.guardrails && (
            <div className="mt-2 bg-sc-bg rounded-lg p-3">
              <div className="text-[11px] font-bold text-sc-text-muted mb-1 flex items-center gap-1">
                <ShieldCheck size={12} /> גבולות והרשאות
              </div>
              <p className="text-[12px] text-sc-text-secondary whitespace-pre-wrap m-0">
                {agent.guardrails}
              </p>
            </div>
          )}
            ...
            <div className="space-y-1.5">
              {agent.discrepancies.map((disc, i) => (
                ...
                >
                  ...
                  <span
                    dir="ltr"
                    className="text-sc-text-secondary text-right"
        ...
        <Section title="גרסה ומודל — מי צריך מה ולמה" icon={<GitBranch size={13} />}>
          <div className="bg-sc-bg rounded-lg p-3 space-y-2">
            <div className="flex items-center gap-2 text-[13px]">
              ...
              <span dir="ltr" className="font-mono text-[12px] text-sc-text-secondary">
                {agent.model ?? '—'}
              </span>
        ...
        {agent.workspace && (
          <div
            dir="ltr"
            className="text-[11px] font-mono text-sc-text-muted pt-2 border-t border-sc-border"
          >
            {agent.workspace}
          </div>
        )}
...
export default function AdminAgents() {
  ...
  const columns: ColumnDef<AgentListRow, unknown>[] = [
    ...
    },
    {
      ...
      header: 'צוות',
      cell: c => (
        <Pill kind={TEAM_PILL[c.getValue() as string] ?? 'neutral'}>
          {TEAM_LABEL[c.getValue() as string] ?? String(c.getValue())}
        </Pill>
      ),
      size: 90,
    },
    ...
    },
    {
      ...
      header: 'גרסה',
      cell: c => (
        <span className="sc-num font-bold text-[12px]">
          {c.getValue() ? `v${c.getValue()}` : '—'}
        </span>
      ),
      size: 60,
    },
    {
      ...
      header: 'מודל',
      cell: c => (
        <span dir="ltr" className="font-mono text-[11px] text-sc-text-secondary">
          {shortModel(c.getValue() as string | null)}
        </span>
      ),
      size: 110,
    },
    {
      ...
      header: 'סקילים',
      cell: c => <span className="sc-num text-[12px]">{c.getValue() as number}</span>,
      size: 55,
    },
    {
    ...
    },
    {
      ...
      cell: c => {
        ...
        return n > 0 ? (
          <span className="inline-flex items-center gap-1 text-sc-danger text-[12px] font-bold">
    ...
    },
    {
      ...
      cell: c => (
        <span className="sc-num text-[12px] text-sc-text-secondary">
          {fmtDate(c.getValue() as string | null)}
        </span>
      ),
      size: 100,
    },
