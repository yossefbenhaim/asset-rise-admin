...
const CAT_LABEL: Record<string, string> = {
  infra: 'תשתיות',
  ai: 'AI ומודלים',
  comms: 'תקשורת (SMS/WhatsApp/מייל)',
...
const STATUS_PILL: Record<string, 'success' | 'info' | 'neutral'> = {
  active: 'success',
  planned: 'info',
  cancelled: 'neutral',
}
...
function ItemDrawer({
  ...
  const inp =
    'mt-1 w-full bg-sc-bg border border-sc-border rounded-sc-input px-3 py-2 text-[13px] text-sc-text'
      ...
      <div className="p-4 space-y-3 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className={lbl}>
            ...
            <input
              type="date"
              className={inp}
              value={form.expected_from ? String(form.expected_from).slice(0, 10) : ''}
              onChange={e => setForm(f => ({ ...f, expected_from: e.target.value || null }))}
            />
          </label>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-sc-border">
          <button
            ...
            onClick={() => {
              if (confirm(`למחוק את ${item.name}?`)) remove.mutate({ id: item.id })
            }}
          >
            <Trash2 size={13} /> מחק
          </button>
            ...
            onClick={() =>
              update.mutate({
                ...
                patch: {
                  ...
                  billing: form.billing as never,
                  usage_note: form.usage_note ?? null,
                  is_estimate: form.is_estimate,
                  expected_from: form.expected_from ?? null,
...
function AddModal({
  ...
  const create = trpc.costs.create.useMutation({
    ...
    onError: e =>
      toast.show(
        e.message?.includes('duplicate') ? 'פריט בשם הזה כבר קיים' : e.message || 'הוספה נכשלה',
      ),
  })
  const inp =
    'mt-1 w-full bg-sc-bg border border-sc-border rounded-sc-input px-3 py-2 text-[13px] text-sc-text'
...
export default function AdminCosts() {
  ...
  const donutData = (s?.byCategory ?? [])
    .map(c => ({ name: CAT_LABEL[c.category] ?? c.category, value: Math.round(c.active) }))
    .filter(d => d.value > 0)
  const barData = (s?.byCategory ?? []).map(c => ({
    cat: CAT_LABEL[c.category] ?? c.category,
    now: Math.round(c.active),
    launch: Math.round(c.active + c.planned),
  }))
  ...
  const columns: ColumnDef<Item, unknown>[] = [
    {
      ...
      cell: c => (
        <div>
          <div className="font-bold text-sc-text">
            {c.row.original.name}
            {c.row.original.is_estimate && (
              <span className="text-sc-warning text-[11px] mr-1">~אומדן</span>
            )}
          </div>
          {c.row.original.provider && (
            <div className="text-[11px] text-sc-text-muted">{c.row.original.provider}</div>
          )}
        </div>
      ),
    },
    {
      ...
      cell: c => (
        <span className="text-[12px]">
          {CAT_LABEL[c.getValue() as string] ?? String(c.getValue())}
        </span>
      ),
    },
    {
      ...
      cell: c => (
        <Pill kind={STATUS_PILL[c.getValue() as string] ?? 'neutral'}>
          {STATUS_LABEL[c.getValue() as string]}
        </Pill>
      ),
    },
    {
      ...
      cell: c => (
        <span dir="ltr" className="text-[13px] font-bold">
          {c.row.original.currency === 'USD' ? '$' : '₪'}
          {Number(c.getValue()).toLocaleString()}
        </span>
      ),
    },
    {
      accessorKey: 'billing',
      header: 'מחזור',
      cell: c => <span className="text-[12px]">{BILLING_LABEL[c.getValue() as string]}</span>,
    },
    {
      id: 'monthly',
      header: 'שווי חודשי',
      cell: c => (
        <span className="text-[13px]">
          {c.row.original.billing === 'one_time' ? '—' : ils(monthlyIls(c.row.original, rate))}
        </span>
      ),
    },
  ]
    ...
    <div className="sc-page">
      <div className="sc-page__head">
        ...
        <Button onClick={() => setAdding(true)}>
          <Plus size={15} /> הוסף עלות
        </Button>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <KpiCard
          label="הוצאה חודשית היום"
          value={ils(s?.activeMonthlyIls ?? 0)}
          icon={<Wallet size={18} />}
          tone="primary"
          index={0}
        />
        <KpiCard
          label="צפי חודשי בהשקה"
          value={ils(s?.launchMonthlyIls ?? 0)}
          icon={<Rocket size={18} />}
          tone="gold"
          index={1}
        />
        <KpiCard
          label="שנתי (מצב נוכחי)"
          value={ils(s?.annualNowIls ?? 0)}
          icon={<TrendingUp size={18} />}
          tone="navy"
          index={2}
        />
        <KpiCard
          label="חד-פעמי מתוכנן"
          value={ils(s?.oneTimePlannedIls ?? 0)}
          icon={<AlertTriangle size={18} />}
          tone={(s?.oneTimePlannedIls ?? 0) > 0 ? 'gold' : 'success'}
          index={3}
        />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
        <DonutChartCard
          title="הוצאה חודשית לפי קטגוריה"
          sub="פריטים פעילים בלבד, בש״ח"
          data={donutData}
          index={0}
        />
        <BarChartCard
          title="היום מול השקה"
          sub="שווי חודשי בש״ח לפי קטגוריה — כולל פריטים מתוכננים"
          data={barData}
          xKey="cat"
          yKey="launch"
          valueFmt={n => ils(n)}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="text-[12px] text-sc-text-secondary flex items-center gap-1">
              ...
              <input
                ...
                step="0.05"
                dir="ltr"
                className="w-16 bg-sc-bg border border-sc-border rounded-sc-input px-2 py-1 text-[12px]"
                value={rate}
                onChange={e => setRate(Number(e.target.value) || 3.7)}
              />
            </label>
          </div>
        }
