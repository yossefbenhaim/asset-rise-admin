...
function Stat({ icon, label, value, tone }: {
  ...
  const cls =
    tone === 'danger' ? 'bg-sc-danger-bg text-sc-danger'
    : tone === 'warning' ? 'bg-sc-warning-bg text-sc-warning'
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <Link to="/processing" className="text-[12px] font-semibold text-sc-primary inline-flex items-center gap-0.5 hover:underline">
          מרכז העיבוד <ChevronLeft size={14} />
        </Link>
      </div>

      <div className="grid grid-cols-3 gap-2">
        <Stat icon={<AlertTriangle size={16} />} label="נכשלו" value={alerts.failedJobs} tone="danger" />
        <Stat icon={<Clock size={16} />} label="תקועים >30 דק׳" value={alerts.stuckJobs} tone="warning" />
