...
export function BuildingProgressPanel({
  ...
  buildingId: string
  onOpenChat?: (userId: string, name: string | null) => void
}) {
  const q = trpc.god.progress.building.useQuery(
  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <div className="flex items-center gap-2 flex-wrap text-[12px]">
        <Pill kind="info">
          {d.totals.done}/{d.totals.total} משימות
        </Pill>
      ...
      {d.baton && (
        <div className="flex items-center justify-between gap-2 rounded-sc-input bg-sc-light-blue/50 px-3 py-2">
          <span className="inline-flex items-center gap-2 text-[12.5px] min-w-0">
            ...
            <span className="font-bold text-sc-text truncate">
              {d.baton.full_name ?? d.baton.email ?? '—'}
            </span>
          ...
          {onOpenChat && (
            <Button
              ...
              onClick={() => onOpenChat(d.baton!.id, d.baton!.full_name)}
              className="shrink-0"
            >
              הודעה
            </Button>
