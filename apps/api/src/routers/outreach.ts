...
export const outreachRouter = router({
  ...
  stats: requireAction('admin.outreach.view').query(async ({ ctx }) => {
    ...
    const daysElapsed = gateStart
      ? Math.floor((Date.now() - new Date(gateStart).getTime()) / 86400000)
      : 0
    ...
    return {
      ...
      followupsDue: rows.filter(
        r => r.next_followup_at && r.next_followup_at <= now && !CALLED.has(r.status),
      ).length,
    }
  }),
  ...
  update: requireAction('admin.outreach.manage')
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        input.patch.status &&
        input.patch.status !== 'new' &&
        input.patch.status !== 'not_relevant'
      ) {
